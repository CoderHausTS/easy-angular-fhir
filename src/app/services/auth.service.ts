...
//parameter and is required if using that parameter. Currently, only the S256 method is supported.
  fetchAuthorizationCode() {
    ...
    const params = new HttpParams({
      fromString: `response_type=${responseType}&client_id=${clientId}&redirect_uri=${redirectUri}&scope=&state=`,
    });

    // this.redirect(this.authorizationURL + '?' + params.toString());
    // console.log('authurl ', this.authorizationURL + '?' + params.toString());
    window.location.href = this.authorizationURL + '?' + params.toString();
    // window.open(this.authorizationURL + '?' + params.toString(), '_blank');
  }
...
//           when using PKCE. This parameter is passed as free text and must match the code_challenge parameter used in your authorization request once it is hashed on the server using the code_challenge_method. This parameter is available starting in the August 2019 version of Epic.
  fetchAuthorizationToken() {
    ...
    // let body = { grant_type : grantType, code: this.authorizeCode, redirect_uri: redirectURI, client_id: clientID};
    let body = `grant_type=${grantType}&code=${this.authorizeCode}&redirect_uri=${redirectURI}&client_id=${clientID}`;
    console.log('body ', body);
    this.http.post(tokenURL, body, httpOptions).subscribe(data => console.log(data));
    // const params = new HttpParams({
    //   fromString: `grant_type=${grantType}&code=${this.authorizeCode}&redirect_uri=${redirectURI}&client_id=${clientID}`,
    // });

    // console.log('authtoken ', tokenURL + '?' + params.toString());
    // this.http.post(tokenURL + '?' + params.toString(), '', httpOptions).subscribe(data => console.log(data));
  }
